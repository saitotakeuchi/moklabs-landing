...
const REQUIRED_VARS = ["RESEND_API_KEY", "FROM_EMAIL", "TO_EMAIL"];
...
if (!fs.existsSync(envLocalPath)) {
  ...
  if (fs.existsSync(envExamplePath)) {
    console.log(
      chalk.blue("💡 Copy .env.example to .env.local and configure it:")
    );
...
REQUIRED_VARS.forEach((varName) => {
  if (process.env[varName]) {
    ...
    const maskedValue =
      value.substring(0, 4) + "*".repeat(Math.min(value.length - 4, 10));
