...
export function DocumentDetail({ documentId, isOpen, onClose }: DocumentDetailProps) {
  const { document: documentData, isLoading, error } = useDocumentDetails({
    documentId,
    includeChunks: true,
    autoFetch: isOpen,
  });
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-lg font-medium text-gray-900">{documentData.title}</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Edital ID</h3>
                  <p className="text-gray-900">{documentData.edital_id}</p>
                </div>

                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Document ID</h3>
                  <p className="text-gray-900 font-mono text-sm">{documentData.id}</p>
                </div>

                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Chunks Count</h3>
                  <p className="text-gray-900">{documentData.chunks_count}</p>
                </div>

                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Embeddings Count</h3>
                  <p className="text-gray-900">{documentData.embeddings_count}</p>
                </div>
              ...
              {/* Sample Chunks */}
              {documentData.sample_chunks && documentData.sample_chunks.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Sample Chunks</h3>
                  <div className="space-y-3">
                    {documentData.sample_chunks.map((chunk, index) => (
                      <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        {chunk.page_number !== undefined && (
                          <div className="text-xs text-gray-500 mb-2">Page {chunk.page_number}</div>
                        )}
              ...
              {/* Metadata */}
              {documentData.metadata && Object.keys(documentData.metadata).length > 0 && (
                <div>
